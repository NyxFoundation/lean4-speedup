...
| **C1 — speculative command elaboration** ([design](docs/c1-wavefront-design.md)) | textual command order treated as advisory for statements, with validation + repair (Tomasulo transfer); zero-rebuild oracle + speculation harness | **Phenomenon harnessed on real code**: depth-1 speculation of real Mathlib commands is 79 % valid and hides 61 % of main-thread time on the hottest module (2,870/4,712 ms; true sequential residue only 1,081 ms); oracle-true ceiling 2.3–2.9×. **v0 implemented in the compiler** (lean4 branch, `Elab.speculateStatements`, default off): producer + keyed-task cache + validated adoption with soundness gates; byte-identical output on Mathlib and Batteries corpora (188/188), adoption rate gate-limited pending telescope-name sharing. |
